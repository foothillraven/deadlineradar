...
def check_demo_locked_email_coverage(repo_root: Path) -> list[str]:
    ...
    allowlisted = {
        ...
        "handleFirm2faVerify": "AuditLab 2FA-4's backup-code-redeemed notice (currently held behind BACKUP_CODE_REDEEMED_EMAIL_ENABLED=false besides) -- structurally unreachable for a demo_locked firm regardless: handleFirm2faEnroll() 400s the WHOLE enrollment request for a demo_locked firm before totp_secret_encrypted is ever set, and this handler's own early guard (member.totp_secret_encrypted required) means a demo firm's member can never reach the backup-code branch at all, same front-door-gated category as issueAndSendFirmMemberInviteEmail above",
    }
