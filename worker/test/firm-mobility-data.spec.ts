...
describe("locked-in dataset facts -- a change here should only come from a real re-sourcing pass", () => {
  ...
  it("exactly these states carry a real source_disagreement note (2026-08-17: WV, AZ, NJ resolved via direct primary-source text)", () => {
    const disagreeing = rows.filter((r) => r.sourceDisagreement !== null).map((r) => r.stateSlug).sort();
    expect(disagreeing).toEqual(
      ["nebraska", "new-hampshire", "new-mexico", "new-york", "ohio", "pennsylvania", "us-virgin-islands", "vermont"].sort()
    );
  });
});
